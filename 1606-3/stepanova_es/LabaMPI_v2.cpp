...
int main(int argc, char *argv[])
{
	...
	int m = argc == 1 ? 4 : std::stod(argv[1]);
	int n = argc == 1 ? 4 : std::stod(argv[2]);
	...
	if (procRank == 0) {
		...
		double *residue = new double[m];
		for (int i = 1; i <= tail; ++i) {
			for (int j = 0; j < m; ++j) {
				residue[j] = Matrх[procNum*m + j];
			}
			MPI_Send(residue, m, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
		}

	}

	if (procRank > 0 && procRank <= n - procNum * k) {
		double *tail = new double[m];
		double res = 0;

		MPI_Recv(tail, m, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
		for (int j = 0; j < m; j++) {
			res += tail[j];
		}
	...
	if (procRank == 0) {
		for (int i = 1; i <= n - procNum * k; i++) {
			MPI_Recv(&result[i - 1 + procNum * k], 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &status);
		}
		if (n < 50)
